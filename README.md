...
*   **Persistent Storage:** Saves enrolled student UIDs to the Arduino's built-in EEPROM, meaning data is not lost when the device loses power[cite: 1].
...
*   **Serial Monitor Commands:** Type `L` or `l` in the Serial Monitor at any time to print a list of all currently enrolled students[cite: 1].
*   **Multi-Sensory Feedback:** Utilizes an I2C LCD screen, a piezo buzzer, and Red/Green LEDs to clearly indicate "Access Granted," "Unknown Card," or "Access Denied" states[cite: 1].
*   **Capacity:** Supports up to 10 unique student cards by default[cite: 1].
...
| :--- | :--- | :--- |
| **Green LED** | Digital Pin 2 | Use 220Ω resistor[cite: 1]. |
| **Red LED** | Digital Pin 3 | Use 220Ω resistor[cite: 1]. |
| **Buzzer** | Digital Pin 4 | Positive leg to Pin 4[cite: 1]. |

## 💻 Software & Libraries
To run this project, you will need the Arduino IDE and the following libraries installed (via the Library Manager):
*   `MFRC522` by GithubCommunity (for the RFID reader)[cite: 1]
*   `LiquidCrystal_I2C` by Frank de Brabander (for the LCD)[cite: 1]
*   `SPI`, `Wire`, and `EEPROM` (Built-in Arduino libraries)[cite: 1]
...
4.  **Open Serial Monitor:** Open the Arduino Serial Monitor and set the baud rate to **9600**[cite: 1]. 
5.  **Initial Scan:** The LCD will display "Scan Card..."[cite: 1]. Tap an RFID card/tag.
...
7.  **Mark Attendance:** Tap the newly enrolled card again. The Green LED will light up, the buzzer will emit a short beep, and the LCD will display "STUDENT 1 PRESENT"[cite: 1].
8.  **List Records:** Type `L` in the Serial Monitor at any time to view all saved student UIDs[cite: 1].
